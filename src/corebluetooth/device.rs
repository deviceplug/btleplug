...
impl BluetoothDevice {
    pub fn new(adapter: Arc<BluetoothAdapter>, uuid: String) -> BluetoothDevice {
        ...
        BluetoothDevice { adapter: adapter.clone(), peripheral: peripheral }
    }
    ...
    fn peripheral_by_uuid(delegate: *mut Object, uuid: &String) -> *mut Object {
        ...
        for i in 0..ns::array_count(keys) {
            ...
            if nsx::string_to_string(uuid_nsstring) == *uuid {
                ...
                return ns::dictionary_objectforkey(data, nsx::string_from_str(bm::PERIPHERALDATA_PERIPHERALKEY));
            }
        }
        nil
    }
    ...
    pub fn get_address(&self) -> Result<String, Box<dyn Error>> {
        ...
        let uuid_string = nsx::string_to_string(ns::uuid_uuidstring(cb::peer_identifier(self.peripheral)));
    ...
    pub fn get_name(&self) -> Result<String, Box<dyn Error>> {
        ...
        let name = if name_nsstring != nil { nsx::string_to_string(name_nsstring) } else { String::from("") };
    ...
    pub fn get_uuids(&self) -> Result<Vec<String>, Box<dyn Error>> {
        ...
        let mut v = vec!();
        let cbuuids_nsarray = ns::dictionary_objectforkey(data, nsx::string_from_str(bm::PERIPHERALDATA_UUIDSKEY));
        if cbuuids_nsarray != nil {
            for i in 0..ns::array_count(cbuuids_nsarray) {
                v.push(cbx::uuid_to_canonical_uuid_string(ns::array_objectatindex(cbuuids_nsarray, i)));
            }
        }
    ...
    pub fn get_gatt_services(&self) -> Result<Vec<String>, Box<dyn Error>> {
        ...
        for i in 0..ns::array_count(services) {
            let uuid_string = cbx::uuid_to_canonical_uuid_string(cb::attribute_uuid(ns::array_objectatindex(services, i)));
            v.push(uuid_string);
        }
