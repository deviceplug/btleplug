...
impl BluetoothGATTCharacteristic {
    pub fn new(service: Arc<BluetoothGATTService>, uuid: String) -> BluetoothGATTCharacteristic {
        ...
        if characteristic == nil {
            warn!("BluetoothGATTCharacteristic::new found no characteristic for UUID {}", uuid);
        }

        BluetoothGATTCharacteristic { service: service.clone(), characteristic: characteristic }
    }
    ...
    pub fn get_uuid(&self) -> Result<String, Box<Error>> {
        ...
        let uuid_string = cbx::uuid_to_canonical_uuid_string(cb::attribute_uuid(self.characteristic));
    ...
    pub fn read_value(&self) -> Result<Vec<u8>, Box<Error>> {
        ...
        let events = bmx::peripheralevents(self.service.device.adapter.delegate, self.service.device.peripheral)?;
        let key = bmx::valueupdatedkey(self.characteristic);
        let t = wait::get_timestamp();

        cb::peripheral_readvalueforcharacteristic(self.service.device.peripheral, self.characteristic);
    ...
    pub fn write_value(&self, values: Vec<u8>) -> Result<(), Box<Error>> {
        ...
        cb::peripheral_writevalue_forcharacteristic(self.service.device.peripheral, ns::data(values.as_ptr(), values.len() as c_uint), self.characteristic);
    ...
    pub fn start_notify(&self) -> Result<(), Box<Error>> {
        ...
        cb::peripheral_setnotifyvalue_forcharacteristic(self.service.device.peripheral, YES, self.characteristic);
        Ok(())
    }
