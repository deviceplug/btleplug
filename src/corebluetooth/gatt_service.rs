...
impl BluetoothGATTService {
    pub fn new(device: Arc<BluetoothDevice>, uuid: String) -> BluetoothGATTService {
        ...
        if service == nil {
            warn!("BluetoothGATTService::new found no service for UUID {}", uuid);
        }

        BluetoothGATTService { device: device.clone(), service: service }
    }
    ...
    pub fn get_includes(&self) -> Result<Vec<String>, Box<dyn Error>> {
        ...
        for i in 0..ns::array_count(includes) {
            v.push(cbx::uuid_to_canonical_uuid_string(cb::attribute_uuid(ns::array_objectatindex(includes, i))));
        }
        Ok(v)
    }
    ...
    pub fn get_gatt_characteristics(&self) -> Result<Vec<String>, Box<dyn Error>> {
        ...
        for i in 0..ns::array_count(chars) {
            v.push(cbx::uuid_to_canonical_uuid_string(cb::attribute_uuid(ns::array_objectatindex(chars, i))));
        }
        Ok(v)
    }
}
